...
<body>
    <div ng-controller="gradesCtrl" ng-init="edit_index=-1">
        <c:choose>
            <%--IF PROFESSOR, APPLY THIS FUNCTIONALITY--%>
            <c:when test="${userType eq 'prof'}">
                <%--Adding an announcement--%>
                <h4 class="panel-title">
                    <button ng-click="displayNewForm(0)" >Add Grade Task</button>
                </h4>
                ...
                <%--<div id="addGradeDiv">--%>
                    <%--<form name="addGrade" ng-submit="addGrade()" id="addGradeForm">--%>
                        ...
                        <%--</div>--%>
                        <%--<div>--%>
                            <%--&lt;%&ndash;hw", "quiz", "ec", "exam", "att", "other&ndash;%&gt;--%>
        ...
        <%--Load the Grade--%>'{{edit_index}}
            <div class="gradeContainer" ng-repeat="gradable in gradables" ng-include="setIndex()"></div>

                    <script type="text/ng-template" id="display">

                        <div ng-if="edit_index===$index">ADD NEW HERE</div>
                         {{edit_index}} {{$index}}
                        <div ng-init="edit_index=-1"></div>

                        <div ng-if="edit_index!=$index">

                            sfhehfwh {{edit_index}}
                            {{gradable.title}} This is the maxgrade: {{gradable.maxGrade}}
                        </div>
                    </script>
                sfhehfwh{{edit_index}}
